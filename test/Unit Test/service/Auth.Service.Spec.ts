...
describe("AuthService Tests", () => {
  ...
  describe("loginWithKakao function test", () => {
    it("basic", async () => {
      ...
      userRepository.findByKakaoId.mockResolvedValueOnce({
        id: userId,
      } as unknown as User); // User found
      (authService as any).generateJwtTokens = jest
        .fn()
        .mockReturnValue(tokens);
      redisService.setValue.mockResolvedValueOnce(undefined);

      const result = await authService.loginWithKakao(kakaoAccessToken);

      expect(result).toEqual(tokens);
      expect(kakaoApiService.bringUserInfo).toHaveBeenCalledWith(
        kakaoAccessToken
      );
      expect(userRepository.findByKakaoId).toHaveBeenCalledWith(
        userInfo.kakaoId
      );
      ...
      expect(redisService.getValue).toHaveBeenCalledWith(String(userId));
    });
  });
  ...
  describe("logout function test", () => {
    it("basic", async () => {
      ...
      redisService.deleteValue.mockResolvedValueOnce(undefined);
      firebaseTokenRepository.deleteTokensByUserId.mockResolvedValueOnce(
        undefined
      );
      ...
      expect(redisService.deleteValue).toHaveBeenCalledWith(userId);
      expect(firebaseTokenRepository.deleteTokensByUserId).toHaveBeenCalledWith(
        userId,
        firebaseToken
      );
    });
    ...
    it("should throw error if token is invalid", async () => {
      ...
      await expect(
        authService.logout(refreshToken, firebaseToken)
      ).rejects.toThrow("Invalid token");
      expect(jwt.decode).toHaveBeenCalledWith(refreshToken);
      expect(redisService.deleteValue).not.toHaveBeenCalled();
      expect(
        firebaseTokenRepository.deleteTokensByUserId
