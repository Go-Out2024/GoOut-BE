...
import { BusStationResult, SubwayStationResult } from '../../../src/dto/values/StationResult';
import { BusStation } from '../../../src/entity/BusStation';
import { BusArrivalInfo, BusStationInfo, Station } from '../../../src/dto/values/BusArrivalInfo';
...
describe('Traffic Service Test', () => {
    ...
    const trafficCollectionRepository = new TrafficCollectionRepository() as jest.Mocked<TrafficCollectionRepository>;
    const trafficCollectionDetailRepository = new TrafficCollectionDetailRepository() as jest.Mocked<TrafficCollectionDetailRepository>;
    const transportationRepository = new TransportationRepository() as jest.Mocked<TransportationRepository>;
    const transportationNumberRepository = new TransportationNumberRepository() as jest.Mocked<TransportationNumberRepository>;
    ...
    describe('penetrateTrafficCollection function test', () => {
        it('basic', async () => {
            ...
            trafficCollectionRepository.insertTrafficCollection.mockResolvedValue(trafficCollcection);
            const verifyInsertTrafficCollectionStatusSpy = jest.spyOn(trafficService as any, 'verifyInsertTrafficCollectionStatus').mockResolvedValue(null);
            const result = await trafficService.penetrateTrafficCollection(collectionInsert, userId);
            expect(result).toEqual(undefined);
            expect(trafficCollectionRepository.insertTrafficCollection).toHaveBeenCalledWith(collectionInsert, userId);
            expect(verifyInsertTrafficCollectionStatusSpy).toHaveBeenCalledWith(collectionInsert, trafficCollcection);
        });
    });
    describe('penetrateTrafficCollectionDetail function test', () => {
        it('should successfully insert traffic collection details', async () => {
            ...
            const insertTrafficCollectionDetailSpy = jest.spyOn(trafficService['trafficCollectionDetailRepository'], 'insertTrafficCollectionDetail').mockResolvedValue(trafficCollectionDetail);
            const insertTransportationSpy = jest.spyOn(trafficService['transportationRepository'], 'insertTransportation').mockResolvedValue(transportation);
            const insertTransportationNumbersSpy = jest.spyOn(trafficService['transportationNumberRepository'], 'insertTransportationNumbers').mockResolvedValue(transportationNumbers);
            await trafficService['penetrateTrafficCollectionDetail'](detailDto, trafficCollection);
            expect(insertTrafficCollectionDetailSpy).toHaveBeenCalledWith(detailDto, trafficCollection);
            ...
            expect(insertTransportationNumbersSpy).toHaveBeenCalledWith(['1001'], expect.anything());
            expect(insertTransportationNumbersSpy).toHaveBeenCalledWith(['1002'], expect.anything());
        });
    });
    ...
    describe('eraseTrafficCollection function test', () => {
        it('basic', async () => {
            ...
            const result = await trafficService.eraseTrafficCollection(userId, collectionId);
            expect(result).toEqual(undefined);
            expect(trafficCollectionRepository.deleteTrafficCollection).toHaveBeenCalledWith(collectionId, userId);
        });
    });
    ...
    describe('modifyTrafficCollectionName function test', () => {
        it('basic', async () => {
            ...
            const userId = 1;
            const result = await trafficService.modifyTrafficCollectionName(collectionNameUpdate, userId);
            expect(result).toEqual(undefined);
            expect(trafficCollectionRepository.updateTrafficCollection).toHaveBeenCalledWith(collectionNameUpdate, userId);
        });
    });
    ...
    describe('modifyTrafficCollectionDetail function test', () => {
        it('basic', async () => {
            ...
            const trafficCollection = { getId: jest.fn().mockReturnValue(1) } as unknown as TrafficCollection;
            trafficCollectionRepository.findTrafficCollectionByCollectionIdAndUserId.mockResolvedValue(trafficCollection);
            trafficCollectionDetailRepository.deleteTrafficCollectionDetailsByCollectionId.mockResolvedValue(undefined);
            const verifyUpdateTrafficCollectionStatusSpy = jest.spyOn(trafficService as any, 'verifyUpdateTrafficCollectionStatus').mockResolvedValue(null);
            const result = await trafficService.modifyTrafficCollectionDetail(collectionDetailUpdate, 1);
            expect(result).toEqual(undefined);
            expect(trafficCollectionRepository.findTrafficCollectionByCollectionIdAndUserId).toHaveBeenCalledWith(1, 1);
            expect(trafficCollectionDetailRepository.deleteTrafficCollectionDetailsByCollectionId).toHaveBeenCalledWith(1);
            expect(verifyUpdateTrafficCollectionStatusSpy).toHaveBeenCalledWith(collectionDetailUpdate, trafficCollection);
        });
    });
    ...
    describe('bringTrafficCollectionsByUserId function test', () => {
        it('basic', async () => {
            ...
            trafficCollectionRepository.findTrafficCollectionsByUserId.mockResolvedValue(collections);
            const result = await trafficService.bringTrafficCollectionsByUserId(userId, currentTime);
            expect(result).toBeDefined();
            expect(trafficCollectionRepository.findTrafficCollectionsByUserId).toHaveBeenCalledWith(userId);
        });
    });
    ...
    describe('bringTrafficCollectionDetailsById function test', () => {
        it('basic', async () => {
            ...
            const collectionId = 1;
            const trafficCollcection = {} as TrafficCollection
            trafficCollectionRepository.findTrafficCollectionDetailsById.mockResolvedValue(trafficCollcection);
            const result = await trafficService.bringTrafficCollectionDetailsById(userId, collectionId);
            expect(result).toEqual(trafficCollcection);
            expect(trafficCollectionRepository.findTrafficCollectionDetailsById).toHaveBeenCalledWith(userId, collectionId);
        });
    });
    ...
    describe('choiceTrafficCollection function test', () => {
        it('basic', async () => {
            const userId = 1;
            const collectionId = 1;
            trafficCollectionRepository.updateAllChoicesToFalse.mockResolvedValue(undefined);
            trafficCollectionRepository.updateChoiceByCollectionId.mockResolvedValue(undefined);
            const result = await trafficService.choiceTrafficCollection(userId, collectionId);
            expect(result).toEqual(undefined);
            expect(trafficCollectionRepository.updateAllChoicesToFalse).toHaveBeenCalledWith(userId);
            expect(trafficCollectionRepository.updateChoiceByCollectionId).toHaveBeenCalledWith(userId, collectionId);
        })
    })
    describe('bringTrafficCollectionsByUserId function test', () => {
        it('should filter by goToWork when current time is before 14:00', async () => {
            ...
            expect(result[1].trafficCollectionDetails[0].getStatus()).toBe('goToWork');
        });
    ...
    it('should handle goHome status and then switch to goToWork if no collection is found for goHome 1', async () => {
        ...
        transportationNumberRepository.findTransportationNumbers.mockResolvedValue([{ getNumbers: jest.fn().mockReturnValue('1001') }] as unknown as TransportationNumber[]);
        const subwayArrivalInfo = [] as SubwayArrivalInfo[];
        jest.spyOn(trafficService as any, 'bringMainSubwayArrivalInfo').mockResolvedValue(subwayArrivalInfo);

        const result = await trafficService.bringMainTrafficCollection(userId);
        
        expect(result.collection).toBeDefined();
        expect(result.result).toBeDefined();
        expect(trafficCollectionRepository.findMainTrafficCollection).toHaveBeenCalledWith(userId, 'goHome');
        expect(trafficCollectionRepository.findMainTrafficCollection).toHaveBeenCalledWith(userId, 'goToWork');
    });
    ...
    it('should handle ErrorResponseDto for Subway arrival info not found 1', async () => {
        ...
        trafficCollectionRepository.findMainTrafficCollection.mockResolvedValueOnce(mockCollection); 
        transportationNumberRepository.findTransportationNumbers.mockResolvedValue([{ getNumbers: jest.fn().mockReturnValue('1001') }] as unknown as TransportationNumber[]);

        const errorResponse = ErrorResponseDto.of<SubwayArrivalInfo>(ErrorCode.NOT_FOUND_SUBWAY_ARRIVAL_INFO);
        jest.spyOn(trafficService as any, 'bringMainSubwayArrivalInfo').mockRejectedValue(errorResponse);

        const result = await trafficService.bringMainTrafficCollection(userId);
        
        expect(result.collection).toBeDefined();
        expect(result.result.getSubwayErrorMessage()).toBe(errorResponse.getMessage());
    ...
    it('should handle Bus transportation and return BusStationResult 1', async () => {
        ...
        trafficCollectionRepository.findMainTrafficCollection.mockResolvedValueOnce(mockCollection); 
        transportationNumberRepository.findTransportationNumbers.mockResolvedValue([{ getNumbers: jest.fn().mockReturnValue('1001') }] as unknown as TransportationNumber[]);
        const mockBusStations = [{ stationName: 'Station 1', busArrivalInfo: [] }] as unknown as BusStationInfo[];
        jest.spyOn(trafficService as any, 'bringMainBusStationsInfo').mockResolvedValue(mockBusStations);
    ...
    describe('changeTrafficRoute function test', () => {
        it('should handle subwayarrinfo when currentStatus is goToWork', async () => {
            ...
            const subwayArrivalInfo = [] as SubwayArrivalInfo[];
            jest.spyOn(trafficService as any, 'bringMainSubwayArrivalInfo').mockResolvedValue(subwayArrivalInfo);
            const result = await trafficService.changeTrafficRoute(userId, currentStatus);
            expect(result.collection).toBeDefined();
            expect(result.result).toBeDefined();
            expect(trafficCollectionRepository.findMainTrafficCollection).toHaveBeenCalledWith(userId, newStatus); //14시 이전엔 goToWork, 14시 이후엔 goHome으로 테스트 해야함.
            expect(trafficService['bringMainSubwayArrivalInfo']).toHaveBeenCalledWith(expect.anything(), ['1001']);
        });
        ...
        it('should handle subwayarrinfo when currentStatus is goHome', async () => {
            ...
            expect(result.collection).toBeDefined();
            expect(result.result).toBeDefined();
            expect(trafficCollectionRepository.findMainTrafficCollection).toHaveBeenCalledWith(userId, newStatus); 
            expect(trafficService['bringMainSubwayArrivalInfo']).toHaveBeenCalledWith(expect.anything(), ['1001']);
        });
        ...
        it('should busarrInfo whern currentStatus is goToWork', async () => {
            ...
            trafficCollectionRepository.findMainTrafficCollection.mockResolvedValueOnce(mockCollection);
            transportationNumberRepository.findTransportationNumbers.mockResolvedValue([{ getNumbers: jest.fn().mockReturnValue('123') }] as unknown as TransportationNumber[]);
            const busStationInfo = [] as BusStationInfo[];
            jest.spyOn(trafficService as any, 'bringMainBusStationsInfo').mockResolvedValue(busStationInfo);
            const result = await trafficService.changeTrafficRoute(userId, currentStatus);
            expect(result.collection).toBeDefined();
            expect(result.result).toBeDefined();
            expect(trafficCollectionRepository.findMainTrafficCollection).toHaveBeenCalledWith(userId, newStatus); // 14시 이전엔 goToWork, 14시 이후엔 goHome으로 테스트 해야함.
            expect(trafficService['bringMainBusStationsInfo']).toHaveBeenCalledWith(expect.anything(), ['123']);
        });
    ...
    describe('bringMainSubwayArrivalInfo function test', () => {
        it('basic', async () => {
            ...
            const bringSubwayArrivalInfoSpy = jest.spyOn(trafficService, 'bringSubwayArrivalInfo').mockResolvedValue(mockSubwayArrivalInfo)
           const result = await (trafficService as any).bringMainSubwayArrivalInfo(departureTransportation, numbers);
           expect(result).toEqual(bringMainSubwayArrivalInfoResponse);
            expect(bringSubwayArrivalInfoSpy).toHaveBeenCalledWith(departureTransportation.getStationName(), numbers);
        });
    });
    ...
    describe('bringMainBusStationsInfo function test', () => {
        it('basic', async () => {
            const departureTransportation = { getStationName: jest.fn().mockReturnValue('Station B') } as unknown as Transportation;
            const bringBusArrivalInfoResponse = [{ "busArrivalInfo": [], "station": undefined }] as unknown as BusStation[]
            const numbers = ['100', '200'];
            const mockBusStations = [{ getStationNum: jest.fn().mockReturnValue(1)}] as unknown as BusStation[];
            const mockBusArrivalInfo = [] as BusArrivalInfo[]
            busStationRepository.findByStationName.mockResolvedValue(mockBusStations);
            const bringBusARrivalInfoSpy = jest.spyOn(trafficService, 'bringBusArrivalInfo').mockResolvedValue(mockBusArrivalInfo);
            const result = await (trafficService as any).bringMainBusStationsInfo(departureTransportation, numbers);
            expect(result).toEqual(bringBusArrivalInfoResponse);
            expect(busStationRepository.findByStationName).toHaveBeenCalledWith('Station B');
    ...
    describe('verifyInsertTrafficCollectionStatus function test', () => {
        it('basic', async () => {
            ...
            const trafficCollection = {} as TrafficCollection;
            const penetrateSpy = jest.spyOn(trafficService as any, 'penetrateTrafficCollectionDetail').mockResolvedValue(null);
            await trafficService.verifyInsertTrafficCollectionStatus(collectionInsert, trafficCollection);
    ...
    describe('verifyUpdateTrafficCollectionStatus function test', () => {
        it('basic', async () => {
            ...
            const trafficCollection = {} as TrafficCollection;
            const penetrateSpy = jest.spyOn(trafficService as any, 'penetrateTrafficCollectionDetail').mockResolvedValue(null);
            await trafficService.verifyUpdateTrafficCollectionStatus(collectionDetailUpdate, trafficCollection);
            expect(penetrateSpy).toHaveBeenCalledTimes(2);
        });
    });
    ...
    describe('bringSubwayArrivalInfo function test', () => {
        it('basic', async () => {
            ...
            const arrivalList = [{statnNm: '녹사평역', subwayId: '1001', trainLineNm: 'Station - C', arvlMsg2: 'string', arvlMsg3: 'string', bstatnNm: 'string', barvlDt: 5 },
                {statnNm: '녹사평역', subwayId: '1003', trainLineNm: 'Station - C', arvlMsg2: 'string', arvlMsg3: 'string', bstatnNm: 'string', barvlDt: 5 }
            ];
            subwayApi.bringSubwayArrivalInfo.mockResolvedValue(arrivalList);
            const result = await trafficService.bringSubwayArrivalInfo(stationName, numbers);
    ...
    describe('bringBusArrivalInfo function test', () => {
        it('basic', async () => {
            ...
            const result = await trafficService.bringBusArrivalInfo(stationNum, numbers);
            expect(result.length).toBe(1);
