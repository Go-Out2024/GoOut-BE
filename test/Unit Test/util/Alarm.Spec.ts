...
describe("Alarm Util Test", () => {
  ...
  describe("handleAlarm function test", () => {
    it("basic", async () => {
      ...
      trafficService.bringMainTrafficCollection.mockResolvedValue(
        transportationData
      );
      const arrivalDatasSpy = jest
        .spyOn(alarm as any, "extractTransportationArrivalInfo")
        .mockReturnValue(arrivalDatas);
      const flagSpy = jest
        .spyOn(alarm as any, "checkTransportationTime")
        .mockReturnValue(["true 10분 (회기)"]);
      const sendPushAlarmSpy = jest.spyOn(alarm as any, "sendPushAlarm");

      await alarm.handleAlarm(datas);

      expect(trafficService.bringMainTrafficCollection).toHaveBeenCalledWith(
        datas[0].user_id
      );
      expect(arrivalDatasSpy).toHaveBeenCalledWith(transportationData.result);
      expect(flagSpy).toHaveBeenCalledWith(arrivalDatas);
      expect(sendPushAlarmSpy).toHaveBeenCalledWith(datas[0].token, [
        "true 10분 (회기)",
      ]);
    });
  });
  ...
  describe("sendPushAlarm function test", () => {
    const flagDatas = ["true 10분 (회기)"];
    it("basic", async () => {
      await alarm["sendPushAlarm"]("engineValue", flagDatas);
      expect(pushNotice).toHaveBeenCalledWith(
        "engineValue",
        "교통수단 알림",
        "등록하신 교통수단의 도착 시간이 (회기)역방면 10분 전입니다."
      );
    });
  });
  ...
  describe("checkTransportationTime function test", () => {
    const time = ["10분 ", "[5]번째"];
    const flagDatas = ["true 10분 ", "true [5]번째 "];
    it("basic", () => {
      const result = alarm["checkTransportationTime"](time);
      expect(result).toEqual(flagDatas);
    });
  });
  ...
  describe("checkTransportationTime function test", () => {
    it("getBusStations undefined", () => {
      const stationResult = {
        getSubwayStation: jest.fn().mockReturnValue({
          getSubwayArrivalInfo: jest
            .fn()
            .mockReturnValue(["result1", "result2"]),
        }),
        getBusStations: jest.fn().mockReturnValue(undefined),
      } as unknown as StationResult;
      const extractSubwayArrivalInfoSpy = jest
        .spyOn(alarm as any, "extractSubwayArrivalInfo")
        .mockReturnValue(["result1", "result2"]);
      const result = alarm["extractTransportationArrivalInfo"](stationResult);
      expect(result).toEqual(["result1", "result2"]);
      expect(extractSubwayArrivalInfoSpy).toHaveBeenCalledWith(
        stationResult.getSubwayStation()
      );
    });
    ...
    it("getBusStations undefined", () => {
      ...
      const extractSubwayArrivalInfoSpy = jest
        .spyOn(alarm as any, "extractBusArrivalInfo")
        .mockReturnValue(["result1", "result2"]);
      const result = alarm["extractTransportationArrivalInfo"](stationResult);
      expect(result).toEqual(["result1", "result2"]);
      expect(extractSubwayArrivalInfoSpy).toHaveBeenCalledWith(
        stationResult.getBusStations()
      );
    });
