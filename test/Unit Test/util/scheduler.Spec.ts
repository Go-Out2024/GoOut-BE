...
import {
  settingRecommendMusic,
  handleAlarm,
} from "../../../src/util/scheduler";
...
describe("scheduler Util Test", () => {
  beforeEach(() => {
    ...
    Container.set(AlarmRepository, {
      findDataForAlarm: jest
        .fn()
        .mockResolvedValue([{ user_id: 1, token: "test-token" }]),
    });
    Container.set(Alarm, {
      handleAlarm: jest.fn(),
    });
  ...
  describe("settingRecommendMusic function test", () => {
    it("should schedule the YouTube music recommendation", async () => {
      const scheduleJobSpy = jest
        .spyOn(schedule, "scheduleJob")
        .mockImplementation((cron, func) => {
          func();
          return {};
        });
  ...
  describe("handleAlarm function test", () => {
    it("should schedule the alarm handling", async () => {
      ...
      expect(alarmServiceMock.handleAlarm).toHaveBeenCalledWith([
        { user_id: 1, token: "test-token" },
      ]);
    });
  });
});
